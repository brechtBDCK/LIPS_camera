...
public:
	...
	*/
    ...
    /**
    @brief new API introduced by LIPS
    used to turn the depth/ir frame synchronization feature on and off.
	@param [in] isEnabled Set to TRUE to enable synchronization, FALSE to disable it
	@returns Status code indicating success or failure of this operation
     */
	Status setDepthIRSyncEnabled(bool isEnabled)
	{
		Status rc = STATUS_OK;

		if (isEnabled)
		{
			rc = (Status)oniDeviceEnableDepthIRSync(m_device);
		}
		else
		{
			oniDeviceDisableDepthIRSync(m_device);
		}

		return rc;
	}

	bool getDepthIRSyncEnabled()
	{
		return oniDeviceGetDepthIRSyncEnabled(m_device) == TRUE;
	}
